...
Terminal client for WudiBuddy Agents server. Connects over WebSocket (`ws://` or `wss://`) to a running headless server.
...
- For all other commands: a running WudiBuddy Agents headless server with URL and token
