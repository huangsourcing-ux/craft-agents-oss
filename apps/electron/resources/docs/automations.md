...
This guide explains how to configure automations in WudiBuddy Agents to automate workflows based on events.
...
Automations allow you to trigger actions automatically when specific events occur in WudiBuddy Agents. You can:
...
### App Events (triggered by WudiBuddy Agents)
...
Send a prompt to WudiBuddy Agents (creates a new session for scheduled prompts).
...
```json
{
  ...
  "method": "POST",
  "body": { "text": "Hello from WudiBuddy Agents!" }
}
```
...
4. **Pair the supergroup.** In WudiBuddy Agents: Settings → Messaging → Telegram → **Pair Supergroup**. Copy the 6-digit code, then in any topic of the supergroup type `/pair <code>`. The bot confirms and the Settings row updates with the group's title.
...
Ask WudiBuddy Agents to validate your automations configuration:
...
2. **Check env vars** — Ensure `CRAFT_WH_*` variables are set in your shell profile and WudiBuddy Agents was restarted after adding them. URLs using `$VAR` templates are validated after variable expansion — if the variable is empty or unset, the URL will be invalid.
