...
// ============================================================
// WudiBuddy Agents Docs Upstream Proxy
// ============================================================
...
async function connectDocsUpstream(): Promise<void> {
  try {
    ...
    console.error(`WudiBuddy Agents Docs proxy connected: ${docsTools.length} tools`);
  } catch (err) {
    console.error(`WudiBuddy Agents Docs proxy connection failed (non-fatal): ${err instanceof Error ? err.message : String(err)}`);
    docsClient = null;
    docsTools = [];
  }
}
...
async function callDocsUpstream(
  ...
  if (!docsClient) {
    return errorResponse(`WudiBuddy Agents Docs server is not connected. Tool '${name}' unavailable.`);
  }
