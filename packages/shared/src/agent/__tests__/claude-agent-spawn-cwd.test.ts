...
describe('extractSdkReportedBinaryPath', () => {
  ...
  it('captures macOS .app bundle paths without truncating at the first dot', () => {
    const msg =
      'Claude Code native binary not found at /Applications/WudiBuddy Agents.app/Contents/Resources/app/node_modules/@anthropic-ai/claude-agent-sdk-binary/claude';
    expect(extractSdkReportedBinaryPath(msg)).toBe(
      '/Applications/WudiBuddy Agents.app/Contents/Resources/app/node_modules/@anthropic-ai/claude-agent-sdk-binary/claude',
    );
  });
